...
## Pick the exception type a Python programmer already expects

There is no `HlmodError` base class you're supposed to catch everything with. A missing type name raises `KeyError`, because that's what a Python programmer already expects from "look this up by name and it isn't there." A missing function name raises `NameError`, an unresolvable `HFUN` type index raises `LookupError`, an out-of-bounds field or virtual-slot index raises `IndexError`, a bad argument raises `TypeError` - the same vocabulary the standard library already uses for the same shape of mistake, chosen deliberately per call site instead of collapsing everything into one generic wrapper exception that tells you nothing until you read the message. The cost is a little more code in the C API surface, one `PyErr_Format` call per failure mode instead of a single catch-all; the payoff is that `except KeyError` and `except TypeError` around hlmod calls behave exactly the way they'd behave around any other Python API, with no framework-specific exception hierarchy to learn first.
