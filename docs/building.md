...
just test-interfaces # implementing a Haxe interface from Python, not just overriding
just test-heaps      # generic hxd.Res/FileSystem overlay hook, not Dead-Cells-specific
...
`just test` runs `test-framework`, `test-bridge`, `test-interfaces`, `test-heaps`, `test-value`, `test-events`, `test-harmony`, `test-devloop`, `test-sdk`, and `test-repl`. `test-editor` and `bench` are deliberately left out of the default run - the first needs a Pyright binary on hand (see the recipe's `pyright="pyright"` argument, e.g. `uvx --from pyright which pyright`), and the second reports timings rather than pass/fail, so bundling it into a suite that's supposed to give you a clean yes/no would be actively misleading.

Every suite that touches the compiled VM (`bridge`, `interfaces`, `heaps`, `value`, `events`, `harmony`, `devloop`, `repl`, `performance`) does the same thing under the hood: it creates its own `tempfile.TemporaryDirectory`, copies in only `mods/hlobj.py`, `mods/hlvalues.py`, `mods/modcore/`, and that suite's own fixture module, compiles a purpose-built Haxe fixture against `hlmod-hl/build/bin/hl` in both debug and release, and greps the fixture's stdout for suite-specific completion markers before declaring victory. None of them touch the repository's real `mods/` directory or load Dead Cells - if a suite passes, it's because *that fixture, in isolation*, produced the expected output, not because something happened to work against whatever mods happen to be lying around on your machine.
...
- **`test-interfaces`** checks `OVERRIDE_PATH_OK`, `NEW_INTERFACE_METHOD_OK`, `SECOND_INSTANCE_OK`, `EXCEPTION_PROPAGATION_OK` - a Python class implementing a Haxe `interface` via `create_subclass`'s `interfaces=`/the `implements=` class-keyword sugar, covering both a class that already declares the interface's methods (the ordinary override path) and one that declares none of them at all (brand-new native proto entries synthesized from the interface's own field signatures).
- **`test-heaps`** additionally copies the shipped `mods/heaps` package itself (not just the framework core) into its temporary mods directory alongside a synthetic `hxd.Res`/`hxd.fs.FileSystem`/`hxd.res.Loader` fixture standing in for real Heaps, then checks `HEAPS_FIXTURE_OK`, `HEAPS_OVERLAY_OVERRIDE_OK`, `HEAPS_OVERLAY_PASSTHROUGH_OK` - a loose override file registered via `heaps.add_overlay_dir` wins for a path it covers, while every other path still falls through untouched to the base game's own `FileSystem`.
