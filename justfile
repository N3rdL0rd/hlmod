...
test: test-framework test-bridge test-value test-events test-harmony test-devloop test-sdk test-repl
...
test-sdk:
    cd sdk && uv run python3 -m unittest discover -s tests
...
pull:
    cd hlmod-hl && git pull origin master
    git pull

push:
    cd hlmod-hl && git push HEAD:master
    git push